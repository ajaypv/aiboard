...
- When you receive a task, IMMEDIATELY begin working on it. DO NOT just create a todo list and stop.
- Your workflow MUST be:
	1. FIRST, create a todo list item for the task you are about to perform using \`update-todo-list\`.
	2. THEN, immediately execute the steps to complete that todo item in the SAME response.
	3. Mark the todo item as done using \`update-todo-list\`.
	4. Continue this pattern until the task is complete.
- Use \`update-todo-list\` events to track progress. You MUST create a todo list item before doing any work.
- CRITICAL: Your response must include actual work (create, update, move, etc.), not just planning.
- The system will automatically continue your work if there are incomplete todo items, so focus on making substantial progress in each response.
