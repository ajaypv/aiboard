...
export class TldrawAgent {
	...
	async prompt(input: AgentInput) {
		...
		if (!scheduledRequest) {
			...
			// If there are outstanding todo items, automatically schedule a continuation request
			scheduledRequest = {
				messages: ['Continue working on the remaining tasks'],
				contextItems: request.contextItems,
...
function requestAgent({ agent, request }: { agent: TldrawAgent; request: AgentRequest }) {
	...
	const requestPromise = (async () => {
		...
		try {
			console.log('🎯 Starting to iterate over streamAgent actions...')
			let actionCount = 0
			for await (const action of streamAgent({ prompt, signal })) {
				actionCount++
				console.log(`🎨 Processing action #${actionCount}:`, action._type, action)
				if (cancelled) break
				editor.run(
					() => {
						const actionUtil = agent.getAgentActionUtil(action._type)
						console.log('🔧 Got action util for:', action._type)

						// helpers the agent's action
						const transformedAction = actionUtil.sanitizeAction(action, helpers)
						console.log('✨ Sanitized action:', transformedAction)
						if (!transformedAction) {
							console.log('⚠️ Action was nullified by sanitize')
							incompleteDiff = null
							return
						}
						...
						console.log('🎬 Calling agent.act() with action:', transformedAction._type)
						const { diff, promise } = agent.act(transformedAction, helpers)
						console.log('✅ agent.act() completed, diff:', diff, 'promise:', promise)
			...
			console.log(`✅ Finished processing ${actionCount} actions`)
			await Promise.all(actionPromises)
		} catch (e) {
			console.error('❌ Error in requestAgent:', e)
...
async function* streamAgent({
	...
	const isLiveAPI = modelName === 'gemini-live-2.5-flash'

	if (isLiveAPI) {
		console.log('🔌 Using WebSocket for Live API')
		...
		ws.onmessage = async (event) => {
			...
			try {
				...
				// Handle action from Live API
				if (data.type === 'action' && data.action) {
					console.log('📦 Received action:', data.action)
					queue.push(data.action)
		...
		// Generator to yield WebSocket messages
		try {
			while (!done || queue.length > 0) {
				if (queue.length > 0) {
					yield queue.shift()!
				} else if (!done) {
					const result = await new Promise<IteratorResult<Streaming<AgentAction>>>((resolve) => {
						resolveNext = resolve
						// Add timeout to prevent infinite waiting
						setTimeout(() => {
							if (resolveNext === resolve) {
								console.log('⏱️ WebSocket timeout, closing')
								done = true
